...
import 'package:store_app_b2b/model/favourite_item_model.dart';
...
class BuyController extends GetxController with GetTickerProviderStateMixin {
  ...
  Future<dynamic> checkProductAvailableInCart(
    ...
    if (userId.value.isNotEmpty) {
      try {
        ...
        String url =
            "${API.checkProductAvailableInCart}/${cartId}/store/${storeId}/user/${userId.value}/skuId/$skuId/check";
  ...
  Future<dynamic> getAddToCartApi(var bodyMap, String? storeId) async {
    ...
    if (userId.value.isNotEmpty) {
      try {
        isAddCartLoading.value = true;
        String url = "${API.getAddToCart}/${userId.value}/store/$storeId";
        ...
        print("response body cart post data${response.body}");
