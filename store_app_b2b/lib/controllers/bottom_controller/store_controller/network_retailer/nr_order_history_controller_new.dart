...
import 'package:store_app_b2b/model/unlisted_order_model_new.dart';
...
class NrOrderHistoryController extends GetxController
  ...
  Future<dynamic> getBuyByDate() async {
    ...
    if (userId.isNotEmpty) {
      try {
        String url =
            '${API.getNrByDate}/$userId?startDate=${DateFormat("yyyy-MM-dd").format(selectFrom ?? DateTime.now())}&endDate=${DateFormat("yyyy-MM-dd").format(selectTo ?? DateTime.now())}&page=0&size=10';
        logs('Url --> $url');
