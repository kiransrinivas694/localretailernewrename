...
class CartController extends GetxController
  ...
  Future<dynamic> getVerifiedProductDataApi() async {
    ...
    if (userId.value.isNotEmpty) {
      try {
        isLoading.value = true;
        logs(
            'getVerifiedProductDataApi message --> ${"${API.getVerifiedProduct}/${userId.value}"}');
        ...
        final response = await http.get(
            Uri.parse("${API.getVerifiedProduct}/${userId.value}"),
            headers: headers);
  ...
  Future<dynamic> getQtyPlushAddToCartApi(Map<String, dynamic> bodyMap) async {
    try {
      print("getQtyPlushAddToCartApi -> ${API.getUpdateQtyAddToCart}");
      ...
      final response = await http.put(Uri.parse(API.getUpdateQtyAddToCart),
  ...
  Future<dynamic> getPlaceOrderApi(Map<String, dynamic> bodyMap) async {
    try {
      ...
      print('getPlaceOrder --> ${API.getPlaceOrder}');
    ...
    isPlaceOrderLoading.value = false;
    print('getPlaceOrder api at last called');
  }

  Future<dynamic> getConfirmOrderApi(Map<String, dynamic> bodyMap) async {
    try {
      // isPlaceOrderLoading.value = true;
      print("printing checkout url ---> ${API.getConfirmOrder}");
    ...
    if (userId.value.isNotEmpty) {
      try {
        ...
        final response = await http.delete(
            Uri.parse(
                "${API.getStoreWiseDeleteProduct}/$cartId/store/$storeId/user/${userId.value}"),
            headers: headers);
        logs(
            'Url --> ${API.getStoreWiseDeleteProduct}/$cartId/store/$storeId/user/${userId.value}');
        print(response.body);
        if (response.statusCode == 200) {
          print(">>>>>>>>${jsonDecode(response.body)}");
          var data = jsonDecode(response.body);
  ...
  Future<dynamic> getSingleItemDeleteFromCartApi(
    ...
    if (userId.value.isNotEmpty) {
      try {
        ...
        logs(
            'getSingleItemDeleteFromCartApi url ---> ${API.getSingleItemDeleteFromCart}/$cartId/store/$storeId/user/${userId.value}/skuId/$skuId');
        final response = await http.delete(
            Uri.parse(
                "${API.getSingleItemDeleteFromCart}/$cartId/store/$storeId/user/${userId.value}/skuId/$skuId"),
            headers: headers);
        print(response.body);
        if (response.statusCode == 200) {
          print(">>>>>>>>${jsonDecode(response.body)}");
  ...
  Future<dynamic> getUnverifiedPlaceOrder() async {
    try {
      logs(
          'getUnverifiedPlaceOrder url ---> ${API.getUnverifiedPlaceOrder}/$userId');
      ...
      final response = await http.put(
          ...
          headers: headers);
      logs('getUnverifiedPlaceOrder response ---> ${response.body}');
      if (response.statusCode == 200) {
        return response.body;
      } else {
        CommonSnackBar.showError('Something went wrong.');
  ...
  Future<dynamic> checkoutGrb() async {
    ...
    try {
      ...
      final response = await http.post(Uri.parse(url),
          headers: headers, body: jsonEncode(tempBody));
      print("printing checkout response ---> ${response.body}");
      if (response.statusCode == 200) {
        print(">>>???????????${response.body}");
        Map<String, dynamic> responseMap = jsonDecode(response.body);
